...
function viewState(){
	$.ajax({
		url : `${mypath}/sboardViewState.do`,
		type : 'get',
		data : {"memId" : memId,
		        "boardId" : boardId},
		success : function(res){
			 $('#htext').text(res.sboard_hit)
			 $('#lktext').text(res.sbstate_like)
			 $('#untext').text(res.sbstate_unlike)
...
function sboardNotify(){
	
	$('#mNotify').on('click', function(){
		const thisNot = $(this);
		$.ajax({
			url : `${mypath}/SboardNotify.do`,
			type : 'get',
