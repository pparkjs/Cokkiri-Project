...
$(function(){
	...
	$('.notice_btn').on('click',function(){
		...
		content = "<%=vo.getNboard_content()%>"
			
		econtent = content.replace(/<br>/g, "\n");
		
		$('#ta1').val("<%=vo.getNboard_title()%>")
		$('#ta2').val(econtent);
...
<body>
 ...
 <div class="con_notice">
	<form action="<%=request.getContextPath()%>/NoticeUpdate.do?nbId=<%=vo.getNboard_id() %>" method="post">
		<div class="notice_header">
			<h1>공지사항 게시글 수정</h1><br>
		</div>
		...
		<div class="notice_bottom">
			<input type="button" class="notice_btn" value="수정하기">
		</div>
	</form>
</div>
